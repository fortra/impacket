...
class SMBTests(RemoteTestCase):
    ...
    def test_reconnectKerberosHashes(self):
        ...
        self.assertEqual(credentials[:6], (self.username, '', self.domain, self.blmhash, self.bnthash, ''))
        self.assertIsNotNone(credentials[6])
        self.assertIsNotNone(credentials[7])
    ...
    def test_loginKerberos(self):
        ...
        credentials = smb.getCredentials()
        self.assertEqual(credentials[:6], (self.username, self.password, self.domain, '', '', ''))
        self.assertIsNotNone(credentials[6])
        self.assertIsNotNone(credentials[7])
        UNC = '\\\\%s\\%s' % (self.machine, self.share)
        smb.connectTree(UNC)
    ...
    def test_loginKerberosAES(self):
        ...
        credentials = smb.getCredentials()
        self.assertEqual(credentials[:6], (self.username, '', self.domain, '', '', self.aes_key_128))
        self.assertIsNotNone(credentials[6])
        self.assertIsNotNone(credentials[7])
        UNC = '\\\\%s\\%s' % (self.machine, self.share)
        smb.connectTree(UNC)
