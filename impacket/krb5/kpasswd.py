...
def _decodePasswordPolicy(ppolicyString):
    ...
    passwordPolicy = {
        ...
        "flags": [flag.name for flag in PasswordPolicyFlags
                    if flag.value & properties[3]],
        }
...
def createKPasswdRequest(principal, domain, newPasswd, tgt, cipher, sessionKey, subKey,
        targetPrincipal=None, targetDomain=None, sequenceNumber=None, now=None, hostname=b"localhost"):
    ...
    encKrbPrivPart["s-address"]["addr-type"] = AddressType.NetBios.value
    ...
    encoded = (struct.pack("!HHH",
                          messageLen,
                          KRB5_KPASSWD_PROTOCOL_VERSION,
                          apReqLen)
                + apReqEncoded
                + krbPrivEncoded)
    return encoded
...
def changePassword(clientName, domain, *args, **kwargs):
    setPassword(clientName, domain, None, None, *args, **kwargs)


def setPassword(clientName, domain, targetName=None, targetDomain=None, newPasswd='',
                oldPasswd='', oldLmhash='', oldNthash='', aesKey='', TGT=None, TGS=None,
    ...
    if TGT is None and os.getenv('KRB5CCNAME'):
        ...
        try:
            ...
            principal = 'krbtgt/{}@{}'.format(domain.upper(), domain.upper())
    ...
    if TGT is None:
        tgt, cipher, oldSessionKey, sessionKey = getKerberosTGT(userName, oldPasswd, domain, oldLmhash, oldNthash, aesKey, kdcHost)
    else:
        tgt = TGT['KDC_REP']
        cipher = TGT['cipher']
