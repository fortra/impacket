...
class TestDot11FrameControlCTS(unittest.TestCase):
    ...
    def test_01_Type(self):
        'Test Type field'
        self.assertEqual(self.cts.get_type(), Dot11.DOT11_TYPE_CONTROL)
        cts=Dot11ControlFrameCTS()
        self.assertEqual(cts.get_type(), Dot11.DOT11_TYPE_CONTROL)

    def test_02_SubType(self):
        'Test SubType field'
        self.assertEqual(self.cts.get_subtype(), Dot11.DOT11_SUBTYPE_CONTROL_CLEAR_TO_SEND)
        cts=Dot11ControlFrameCTS()
        self.assertEqual(cts.get_subtype(), Dot11.DOT11_SUBTYPE_CONTROL_CLEAR_TO_SEND)
    
    def test_03_TypeSubtype(self):
        'Test Type and SubType field'
        self.assertEqual(self.cts.get_type_n_subtype(), Dot11.DOT11_TYPE_CONTROL_SUBTYPE_CLEAR_TO_SEND)
        cts=Dot11ControlFrameCTS()
        self.assertEqual(cts.get_type_n_subtype(), Dot11.DOT11_TYPE_CONTROL_SUBTYPE_CLEAR_TO_SEND)
    ...
    def test_07_FCS(self):
        ...
        fcs=self.cts.get_fcs()
        self.assertEqual(fcs,   0x2B8A6517)
        self.cts.set_fcs(0x44332211)
        self.assertEqual(self.cts.get_fcs(), 0x44332211)
