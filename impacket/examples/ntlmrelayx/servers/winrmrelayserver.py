...
class WinRMRelayServer(Thread):
    ...
    class HTTPHandler(http.server.SimpleHTTPRequestHandler):
        ...
        def do_REDIRECT(self, proxy=False):
            LOG.info('WinRM(%s): redirecting target [307] (%s)' % (self.server.server_address[1], self.path.lower()))
        ...
        def do_GET(self):
            LOG.info('WinRM(%s): Client requested path: %s (GET)' % (self.server.server_address[1], self.path.lower()))
            return self.do_GETPOST()

        def do_POST(self):
            LOG.info('WinRM(%s): Client requested path: %s (POST)' % (self.server.server_address[1], self.path.lower()))
            return self.do_GETPOST()
        ...
        def do_GETPOST(self):
            if self.path.lower() != '/wsman':
              LOG.info('WinRM(%s): skipping path: %s' % (self.server.server_address[1], self.path.lower()))
              self.send_not_found()
              return
                
            LOG.info("WinRM(%s): client: %s server: %s" % (self.server.server_address[1], self.client_address[0], self.server.server_address[0])) 
            ...
            token, messageType = self.strip_blob(proxy)
            LOG.info('WinRM(%s): token: %s messageType: %s' % (self.server.server_address[1], token, messageType))
