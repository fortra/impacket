...
class DNS(ProtocolPacket):
    ...
    def parseCompressedMessage(self, buf, offset=0):
        ...
        byte = struct.unpack("B", buf[offset].encode("latin1"))[0]
        #  if the first two bits are ones (11000000=0xC0), the next bits are the offset
        if byte & 0xC0 == 0xC0:
            # It's a pointer
            pointer = struct.unpack("!H", buf[offset:offset+2].encode("latin1"))[0] # network unsigned short
...
if __name__ == "__main__":
    pkts = [
            "\x6a\x8c\x01\x00\x00\x01\x00\x00\x00\x00\x00\x00\x03\x77\x77\x77\x05\x74\x61\x72\x74\x61\x03\x63\x6f\x6d\x00\x00\x01\x00\x01",
