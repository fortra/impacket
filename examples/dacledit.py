...
#
# SECUREAUTH LABS. Copyright (C) 2024 SecureAuth Corporation. All rights reserved.
#
...
class DACLedit(object):
        ...
        if self.principal_SID is None and self.principal_sAMAccountName is not None or self.principal_DN is not None:
            ...
            if self.principal_sAMAccountName is not None:
                ...
                self.ldap_session.search(
                    ...
                    "(sAMAccountName=%s)" % escape_filter_chars(_lookedup_principal),
                    attributes=["objectSid"],
                )
            elif self.principal_DN is not None:
                _lookedup_principal = self.principal_DN
                self.ldap_session.search(
                    self.domain_dumper.root, "(distinguishedName=%s)" % _lookedup_principal, attributes=["objectSid"]
                )
            try:
    ...
    def write(self):
        ...
        if self.rights == "FullControl" and self.rights_guid is None:
            logging.debug(
                "Appending ACE (%s --(FullControl)--> %s)" % (self.principal_SID, format_sid(self.target_SID))
            )
            self.principal_security_descriptor["Dacl"].aces.append(
            ...
            for rights_guid in self.build_guids_for_rights():
                logging.debug(
                    "Appending ACE (%s --(%s)--> %s)" % (self.principal_SID, rights_guid, format_sid(self.target_SID))
                )
                self.principal_security_descriptor["Dacl"].aces.append(
    ...
    def remove(self):
        ...
        for ace in self.principal_security_descriptor["Dacl"].aces:
            ...
            for compare_ace in compare_aces:
                ...
                if (
                    ace["AceType"] == compare_ace["AceType"]
                    ...
                    and ace["Ace"]["Sid"]["IdentifierAuthority"]["Value"]
                    == compare_ace["Ace"]["Sid"]["IdentifierAuthority"]["Value"]
                ):
    ...
    def backup(self):
        ...
        if not self.filename:
            ...
            if os.path.exists(self.filename):
                logging.info(
                    "File %s already exists, I'm refusing to overwrite it, setting another filename" % self.filename
                )
                self.filename = "dacledit-%s.bak" % datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    def search_target_principal_security_descriptor(self):
        ...
        if self.target_sAMAccountName is not None:
            ...
            self.ldap_session.search(
                self.domain_dumper.root,
                "(objectSid=%s)" % _lookedup_principal,
                attributes=["nTSecurityDescriptor"],
                controls=controls,
            )
            ...
            self.ldap_session.search(
                self.domain_dumper.root,
                "(distinguishedName=%s)" % _lookedup_principal,
                attributes=["nTSecurityDescriptor"],
                controls=controls,
            )
        try:
    ...
    def get_user_info(self, samname):
        self.ldap_session.search(
            self.domain_dumper.root, "(sAMAccountName=%s)" % escape_filter_chars(samname), attributes=["objectSid"]
        )
        try:
    ...
    def create_object_ace(self, privguid, sid, ace_type):
        ...
        if privguid == RIGHTS_GUID.WriteMembers.value:
            acedata["Mask"]["Mask"] = (
                ldaptypes.ACCESS_ALLOWED_OBJECT_ACE.ADS_RIGHT_DS_READ_PROP
...
def parse_args():
    ...
    principal_parser = parser.add_argument_group(
        "principal",
        description="Object, controlled by the attacker, to reference in the ACE to create or to filter when printing a DACL",
    )
    principal_parser.add_argument(
        "-principal", dest="principal_sAMAccountName", metavar="NAME", type=str, required=False, help="sAMAccountName"
    )
    principal_parser.add_argument(
        "-principal-sid", dest="principal_SID", metavar="SID", type=str, required=False, help="Security IDentifier"
    )
    principal_parser.add_argument(
        "-principal-dn", dest="principal_DN", metavar="DN", type=str, required=False, help="Distinguished Name"
    )

    target_parser = parser.add_argument_group("target", description="Principal object to read/edit the DACL of")
    target_parser.add_argument(
        "-target", dest="target_sAMAccountName", metavar="NAME", type=str, required=False, help="sAMAccountName"
    )
    target_parser.add_argument(
        "-target-sid", dest="target_SID", metavar="SID", type=str, required=False, help="Security IDentifier"
    )
    target_parser.add_argument(
        "-target-dn", dest="target_DN", metavar="DN", type=str, required=False, help="Distinguished Name"
    )

    dacl_parser = parser.add_argument_group("dacl editor")
    dacl_parser.add_argument(
        "-action",
        choices=["read", "write", "remove", "backup", "restore"],
        nargs="?",
        default="read",
        help="Action to operate on the DACL",
    )
    dacl_parser.add_argument(
        "-file", dest="filename", type=str, help="Filename/path (optional for -action backup, required for -restore))"
    )
    dacl_parser.add_argument(
        "-ace-type",
        ...
        nargs="?",
        default="allowed",
        help="The ACE Type (access allowed or denied) that must be added or removed (default: allowed)",
    )
    dacl_parser.add_argument(
        "-rights",
        choices=["FullControl", "ResetPassword", "WriteMembers", "DCSync"],
        nargs="?",
        default="FullControl",
        help="Rights to write/remove in the target DACL (default: FullControl)",
    )
    dacl_parser.add_argument("-rights-guid", type=str, help="Manual GUID representing the right to write/remove")
    dacl_parser.add_argument(
        "-inheritance",
        action="store_true",
        help="Enable the inheritance in the ACE flag with CONTAINER_INHERIT_ACE and OBJECT_INHERIT_ACE. Useful when target is a Container or an OU, "
...
def init_ldap_connection(target, tls_version, args, domain, username, password, lmhash, nthash):
    ...
    if args.k:
        ...
        ldap3_kerberos_login(
            ldap_session, target, username, password, domain, lmhash, nthash, args.aesKey, kdcHost=args.dc_ip
        )
    elif args.hashes is not None:
        ldap_session = ldap3.Connection(
            ldap_server, user=user, password=lmhash + ":" + nthash, authentication=ldap3.NTLM, auto_bind=True
        )
    else:
        ldap_session = ldap3.Connection(
            ldap_server, user=user, password=password, authentication=ldap3.NTLM, auto_bind=True
        )
...
def main():
    ...
    if (
        args.action == "write"
        and args.principal_sAMAccountName is None
        and args.principal_SID is None
        and args.principal_DN is None
    ):
