    ...
    def dump(self):
        try:
            if self.__remoteAddr.upper() == 'LOCAL' and self.__username == '':
                ...
                if self.__samHive is not None:
                    self._dump_sam(bootKey)
                if self.__securityHive is not None:
                    self._dump_lsa(bootKey)
            else:
                self.__isRemote = True
                bootKey = None
                try:
