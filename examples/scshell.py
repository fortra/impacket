...
class SCSHELL:
    ...
    def run(self, remoteName, remoteHost, serviceName, noCmd):
        exitCli = False
	...
	self.__scmr.set_auth_level(ntlm.NTLM_AUTH_PKT_PRIVACY)
        ...
        while not exitCli:
          ...
          if not userCommand == 'exit':
            ...
            resp = scmr.hRChangeServiceConfigW(self.__scmr, serviceHandle, scmr.SERVICE_NO_CHANGE, scmr.SERVICE_DEMAND_START, scmr.SERVICE_ERROR_IGNORE, userCommand, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)
            ...
            resp = scmr.hRChangeServiceConfigW(self.__scmr, serviceHandle, scmr.SERVICE_NO_CHANGE, scmr.SERVICE_DEMAND_START, scmr.SERVICE_ERROR_IGNORE, binaryPath, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-no-cmd', action='store', default=False, help='Prepend C:\windows\system32\cmd.exe /c in front of your command')
    ...
    try:
        executer = SCSHELL(username, password, domain, options.hashes, options.aesKey, options.k,
                           options.dc_ip)
        executer.run(remoteName, options.target_ip, options.service_name, options.no_cmd)
    except Exception as e:
