...
#
# Description:
#   This script can be used in order to modify file attributes - utilizing pure SMB.
#
# Author:
...
FILE_ATTRIBUTE_DIRECTORY                = 0x00000010 # Cannot be set.
...
def calculate_attributes(options: argparse.Namespace) -> int:
    pass

def attrib_query(connection: smbconnection.SMBConnection, tid: int, fid: int) -> int:
...
def attrib_set(connection: smbconnection.SMBConnection, tid: int, fid: int, attribs: int):
    if connection.getDialect() == SMB_DIALECT:
        info_data = SMBSetFileBasicInfo()
        info_data['ExtFileAttributes'] = attribs
        fileInfoClass = SMB_SET_FILE_BASIC_INFO
    else:
        info_data = FILE_BASIC_INFORMATION()
        info_data['FileAttributes'] = attribs
        fileInfoClass = SMB2_FILE_BASIC_INFO
    
    connection.setInfo(tid, fid, fileInfoClass, info_data)

def attrib_unset(connection: smbconnection.SMBConnection, tid: int, fid: int, attribs: int):
    current_attributes = attrib_query(connection, tid, fid)
    # TODO: Update attributes and remove the ones that need to be removed.
    updated_attributes = None
    attrib_set(connection, tid, fid, updated_attributes)
...
def main():
    ...
    parser.add_argument("share", type=str, help="The share in which the desired file to query/modify resides.")
    parser.add_argument("path", type=str, help="The path of the desired file to query/set/unset attributes to.")
    ...
    group.add_argument('-p', '--port', default=445, type=int, metavar="destination port", help='Destination port to connect to SMB Server')

    subcommands = parser.add_subparsers(dest="action")
    
    query_parser = subcommands.add_parser("query")
    
    set_parser = subcommands.add_parser("set", add_help=False)    
    # Attribute Flags
    ...
    unset_parser = subcommands.add_parser("unset", add_help=False)    
    unset_parser.add_argument('-r', '--readonly', dest='readonly', action='store_true', help="A file or directory that is read-only. For a file, applications can read the file but cannot write to it or delete it. For a directory, applications cannot delete it, but applications can create and delete files from that directory.")
    unset_parser.add_argument('-h', '--hidden', dest='hidden', action='store_true', help="A file or directory that is hidden. Files and directories marked with this attribute do not appear in an ordinary directory listing.")
    unset_parser.add_argument('-s', '--system', dest='system', action='store_true', help="A file or directory that the operating system uses a part of or uses exclusively.")
    unset_parser.add_argument('-a', '--archive', dest='archive', action='store_true', help="A file or directory that requires to be archived. Applications use this attribute to mark files for backup or removal.")
    unset_parser.add_argument('-t', '--temporary', dest='temporary', action='store_true', help="A file that is being used for temporary storage. The operating system can choose to store this file's data in memory rather than on mass storage, writing the data to mass storage only if data remains in the file when the file is closed.")
    unset_parser.add_argument('-c', '--compressed', dest='compressed', action='store_true', help="A file or directory that is compressed. For a file, all of the data in the file is compressed. For a directory, compression is the default for newly created files and subdirectories.")
    unset_parser.add_argument('-o', '--offline', dest='offline', action='store_true', help="The data in this file is not available immediately. This attribute indicates that the file data is physically moved to offline storage. This attribute is used by Remote Storage, which is hierarchical storage management software.")
    unset_parser.add_argument('-e', '--encrypted', dest='encrypted', action='store_true', help="A file or directory that is encrypted. For a file, all data streams in the file are encrypted. For a directory, encryption is the default for newly created files and subdirectories.")
    unset_parser.add_argument('-p', '--pinned', dest='pinned', action='store_true', help="This attribute indicates user intent that the file or directory should be kept fully present locally even when not being actively accessed. This attribute is for use with hierarchical storage management software.")
    unset_parser.add_argument('-u', '--unpinned', dest='unpinned', action='store_true', help="This attribute indicates that the file or directory should not be kept fully present locally except when being actively accessed. This attribute is for use with hierarchical storage management software.")
    unset_parser.add_argument('--all', dest='all', action='store_true', help="Unset all file attributes, restoring the file to be a normal file.")
    ...
    attribs = calculate_attributes(options)
    ...
    try:
        ...
        else: # For set and unset actions
            desiredAccess = FILE_WRITE_ATTRIBUTES
        
        fid = connection.openFile(tid, path, desiredAccess=desiredAccess)
        
        try:
            if options.action == 'query':
                print(attrib_query(connection, tid, fid))
            elif options.action == 'set':
                attrib_set(connection, tid, fid, attribs)
            elif options.action == 'unset':
                attrib_unset(connection, tid, fid, attribs)
        finally:
