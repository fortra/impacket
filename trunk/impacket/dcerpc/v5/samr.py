...
class GROUP_MEMBERSHIP(NDR):
    structure = (
        ('RelativeId',ndr.NDRLONG),
        ('Attributes',ndr.NDRLONG),
    )
...
class PGROUP_MEMBERSHIP(ndr.NDRPointer):
    ...
    def __init__(self, data = None, isNDR64 = False):
        ...
        self.fields['Data'].item = GROUP_MEMBERSHIP
        if data is not None:
            self.fromString(data)

class SAMPR_GET_GROUPS_BUFFER(NDR):
    structure = (
        ('MembershipCount',ndr.NDRLONG),
        ('Groups',PGROUP_MEMBERSHIP),
    )
...
class SAMPR_SR_SECURITY_DESCRIPTOR(NDR):
    structure = ( 
        ('Length',ndr.NDRLONG),
        ('SecurityDescriptor', ndr.PNDRUniConformantVaryingArray),
    )

class SAMPR_LOGON_HOURS(NDR):
    structure = (
        ('UnitsPerWeek', ndr.NDRLONG),
        ('LogonHours', ndr.PNDRUniConformantVaryingArray),
    )

class SAMPR_USER_ALL_INFORMATION(NDR):
    structure = (
        ('LastLogon'           ,ndr.NDRHYPER),
        ('LastLogoff'          ,ndr.NDRHYPER),
        ('PasswordLastSet'     ,ndr.NDRHYPER),
        ('AccountExpires'      ,ndr.NDRHYPER),
        ('PasswordCanChange'   ,ndr.NDRHYPER),
        ('PasswordMustChange'  ,ndr.NDRHYPER),
        ('UserName'            ,ndr.UNICODE_STRING),
        ('FullName'            ,ndr.UNICODE_STRING),
        ('HomeDirectory'       ,ndr.UNICODE_STRING),
        ('HomeDirectoryDrive'  ,ndr.UNICODE_STRING),
        ('ScriptPath'          ,ndr.UNICODE_STRING),
        ('ProfilePath'         ,ndr.UNICODE_STRING),
        ('AdminComment'        ,ndr.UNICODE_STRING),
        ('WorkStations'        ,ndr.UNICODE_STRING),
        ('UserComment'         ,ndr.UNICODE_STRING),
        ('Parameters'          ,ndr.UNICODE_STRING),
        ('LmOwfPassword'       ,ndr.UNICODE_STRING),
        ('NtOwfPassword'       ,ndr.UNICODE_STRING),
        ('PrivateData'         ,ndr.UNICODE_STRING),
        ('SecurityDescriptor'  ,SAMPR_SR_SECURITY_DESCRIPTOR),
        ('UserId'              ,ndr.NDRLONG),
        ('PrimaryGroupId'      ,ndr.NDRLONG),
        ('UserAccountControl'  ,ndr.NDRLONG),
        ('WhichFields'         ,ndr.NDRLONG),
        ('LogonHours'          ,SAMPR_LOGON_HOURS),
        ('BadPasswordCount'    ,ndr.NDRSHORT),
        ('LogonCount'          ,ndr.NDRSHORT),
        ('CountryCode'         ,ndr.NDRSHORT),
        ('CodePage'            ,ndr.NDRSHORT),
        ('LmPasswordPresent'   ,ndr.NDRSMALL),
        ('NtPasswordPresent'   ,ndr.NDRSMALL),
        ('PasswordExpired'     ,ndr.NDRSMALL),
        ('PrivateDataSensitive',ndr.NDRSMALL),
    )
...
class SamrQueryInformationUser(NDRCall):
    structure = (
        ('UserHandle', SAMPR_HANDLE),
        ('UserInformationClass', ndr.NDRLONG),
    )
...
class DCERPCSamr:
    ...
    def queryuserinfo(self,context_handle):
        ...
        data = self._dcerpc.recv()
        nose = SamrQueryInformationUserResponse(data)
        nose.dump()
    ...
    def SamrGetGroupsForUser(self, userHandle):
        getGroups = SAMRGetGroupsForUser()
        getGroups['UserHandle'] = userHandle
        ans = self.doRequest(getGroups)
        packet = SAMRGetGroupsForUserResponse(ans)
        return packet
