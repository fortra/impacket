...
class TestDot11FrameControlCFEnd(unittest.TestCase):
    ...
    def test_01_Type(self):
        'Test Type field'
        self.assertEqual(self.cfend.get_type_field(), Dot11.DOT11_TYPE_CONTROL)
        cfend=Dot11ControlFrameCFEnd()
        self.assertEqual(cfend.get_type_field(), Dot11.DOT11_TYPE_CONTROL)

    def test_02_SubType(self):
        'Test SubType field'
        self.assertEqual(self.cfend.get_subtype_field(), Dot11.DOT11_SUBTYPE_CONTROL_CF_END)
        cfend=Dot11ControlFrameCFEnd()
        self.assertEqual(cfend.get_subtype_field(), Dot11.DOT11_SUBTYPE_CONTROL_CF_END)
    
    def test_03_TypeSubtype(self):
        'Test Type and SubType field'
        self.assertEqual(self.cfend.get_type_n_subtype_field(), Dot11.DOT11_TYPE_CONTROL_SUBTYPE_CF_END)
        cfend=Dot11ControlFrameCFEnd()
        self.assertEqual(cfend.get_type_n_subtype_field(), Dot11.DOT11_TYPE_CONTROL_SUBTYPE_CF_END)
    ...
    def test_05_Duration(self):
        ...
        self.assertEqual(self.cfend.get_duration_field(), 0x00)
        self.cfend.set_duration_field(0x1234)
        self.assertEqual(self.cfend.get_duration_field(), 0x1234)
    
    def test_06_RA(self):
        'Test RA field'
        
        ra=self.cfend.get_ra_field()
        self.assertEqual(ra.tolist(), [0xff,0xff,0xff,0xff,0xff,0xff])
        ra[0]=0x12
        ra[5]=0x34
        self.cfend.set_ra_field(ra)
        self.assertEqual(self.cfend.get_ra_field().tolist(), [0x12,0xff,0xff,0xff,0xff,0x34])

    def test_07_BSSID(self):
        'Test BSS ID field'
        
        bssid=self.cfend.get_bssid_field()
        self.assertEqual(bssid.tolist(), [0x00,0x19,0xe0,0x98,0x04,0xd4])
        bssid[0]=0x12
        bssid[5]=0x34
        self.cfend.set_bssid_field(bssid)
        self.assertEqual(self.cfend.get_bssid_field().tolist(), [0x12,0x19,0xe0,0x98,0x04,0x34])

    def test_08_FCS(self):
        'Test FCS field'
        
        fcs=self.cfend.get_fcs_field()

        self.assertEqual(fcs, 0xad9c3cc0)
        self.cfend.set_fcs_field(0x44332211)
        self.assertEqual(self.cfend.get_fcs_field(), 0x44332211)
