...
class TestDot11FrameControlPSPoll(unittest.TestCase):
    ...
    def test_01_Type(self):
        'Test Type field'
        self.assertEqual(self.pspoll.get_type_field(), Dot11.DOT11_TYPE_CONTROL)
        pspoll=Dot11ControlFramePSPoll()
        self.assertEqual(pspoll.get_type_field(), Dot11.DOT11_TYPE_CONTROL)

    def test_02_SubType(self):
        'Test SubType field'
        self.assertEqual(self.pspoll.get_subtype_field(), Dot11.DOT11_SUBTYPE_CONTROL_POWERSAVE_POLL)
        pspoll=Dot11ControlFramePSPoll()
        self.assertEqual(pspoll.get_subtype_field(), Dot11.DOT11_SUBTYPE_CONTROL_POWERSAVE_POLL)
    
    def test_03_TypeSubtype(self):
        'Test Type and SubType field'
       
        self.assertEqual(self.pspoll.get_type_n_subtype_field(), Dot11.DOT11_TYPE_CONTROL_SUBTYPE_POWERSAVE_POLL)
        pspoll=Dot11ControlFramePSPoll()
        self.assertEqual(pspoll.get_type_n_subtype_field(), Dot11.DOT11_TYPE_CONTROL_SUBTYPE_POWERSAVE_POLL)
    ...
    def test_05_AID(self):
        ...
        self.assertEqual(self.pspoll.get_aid_field(), 0x181)
        self.pspoll.set_aid_field(0x1234)
        self.assertEqual(self.pspoll.get_aid_field(), 0x1234)
    
    def test_06_BSSID(self):
        'Test BSS ID field'
        
        bssid=self.pspoll.get_bssid_field()
        self.assertEqual(bssid.tolist(), [0x00,0x08,0x54,0xac,0x2f,0x85])
        bssid[0]=0x12
        bssid[5]=0x34
        self.pspoll.set_bssid_field(bssid)
        self.assertEqual(self.pspoll.get_bssid_field().tolist(), [0x12,0x08,0x54,0xac,0x2f,0x34])

    def test_07_TA(self):
        'Test TA field'
        
        ta=self.pspoll.get_ta_field()
        self.assertEqual(ta.tolist(), [0x00,0x23,0x4d,0x09,0x86,0xfe])
        ta[0]=0x12
        ta[5]=0x34
        self.pspoll.set_ta_field(ta)
        self.assertEqual(self.pspoll.get_ta_field().tolist(), [0x12,0x23,0x4d,0x09,0x86,0x34])

    def test_08_FCS(self):
        'Test FCS field'
        
        fcs=self.pspoll.get_fcs_field()
        
        self.assertEqual(fcs, 0x99754373)
        self.pspoll.set_fcs_field(0x44332211)
        self.assertEqual(self.pspoll.get_fcs_field(), 0x44332211)
    ...
    def test_10_AutoChecksum(self):
        ...
        self.pspoll.set_duration_field(0x1234)
        frame=self.pspoll.get_packet()
        fcs=self.pspoll.get_fcs_field()
